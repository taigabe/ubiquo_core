...
class SettingTest < ActiveSupport::TestCase
  ...
  def test_setting_subtypes
    {SymbolSetting => :s, 
      ...
      assert_difference ["#{subtype.name}.count", 'Setting.count'] do
        setting = create_setting(subtype, :key => key, :value => value)
        assert !setting.new_record?, "#{setting.errors.full_messages.to_sentence}"
        assert setting.type == subtype.name
        assert setting.class == subtype
      end
    end
  end

  def test_should_not_create_values_for_not_existent_settings
     assert_no_difference ['Setting.count', 'StringSetting.count'] do
     setting = create_setting(StringSetting, :key => "foooo")
     assert setting.errors.on(:key)
   end
  ...
  def test_requires_existent_context
    assert_no_difference ['Setting.count', 'StringSetting.count'] do
      setting = create_setting(StringSetting, :context => "non-existant")
      assert setting.errors.on(:context)    
    end
  end

  def test_should_require_key
    assert_no_difference ['Setting.count', 'StringSetting.count'] do
      setting = create_setting(StringSetting, :key => "")
      assert setting.errors.on(:key)
    end
  end
  ...
  def test_should_require_value_if_not_nullable
    ...
    assert_no_difference ['Setting.count', 'StringSetting.count'] do
      setting = create_setting(StringSetting, :key => key)
      assert setting.errors.on(:value)    
    end
    ...
    assert_difference ['Setting.count', 'StringSetting.count'] do
      setting = create_setting(StringSetting, :key => key)
      assert !setting.new_record?, "#{setting.errors.full_messages.to_sentence}"
    end
  end
