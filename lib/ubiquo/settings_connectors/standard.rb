            ...
            # only used by plugin or application, not for override
              ...
              if block_given?
                ...
                if !check_valid_name(name)
                  raise Ubiquo::Settings::InvalidOptionName, "Setting #{name} has an invalid name"
                end

                if settings[self.current_context].include?(name.to_sym) && !options.delete(:is_a_connector_reload)
                  raise Ubiquo::Settings::AlreadyExistingOption, "Setting #{name} already exists"
                end
